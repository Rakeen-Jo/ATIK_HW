...
{
#if 0 // 23. 3. 28 By RHJo, Modify to running Interrupt with hardware serial.
...
#if ARDUINO_USB_CDC_ON_BOOT //Serial used for USB CDC
    if(ser_debug.available()) serialEvent();
#else
    if(ser_debug.available()) serialEvent();
#endif
#if SOC_UART_NUM > 1
    if(ser_rs232_0.available()) serialEvent1();
#endif
#if SOC_UART_NUM > 2
    if(ser_rs232_1.available()) serialEvent2();
#endif
